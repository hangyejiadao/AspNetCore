...
namespace Dos.ORM.Web.Areas.MsSys.Controllers
{
    ...
    {
        [Ninject.Inject]
        private ICON_AreaCodeData ConAreaCode { get; set; }
        ...
        /// </summary>
        /// <returns></returns>
        public JsonResult GetUserTheme()
        {
        ...
        {
            var msSysUser = MsSysUserAuth.Get();
            var msSysUserIsNotNull = msSysUser != null && msSysUser.Operator != null;

            var operRet = new OperateModel
            {
                Result = msSysUserIsNotNull ? OperateRetType.Success : OperateRetType.Fail,
                Msg = !msSysUserIsNotNull ? "登录已过期，请重新登陆！" : "",
                Data = msSysUserIsNotNull ? msSysUser.Operator : null
            };
        ...
        /// 
        /// </summary>
        /// <param name="type">1：所有数据（包括“请选择...”）、2：只有“请选择...”</param>
        /// <param name="level"></param>
        /// <param name="areaCode"></param>
        /// <returns></returns>
        [HttpPost]
        public JsonResult GetAreaCode(int type, int level, int areaCode)
        {
            var retList = new List<CON_AreaCode> { new CON_AreaCode { AreaCode = -1, AreaName = "请选择..." } };

            if (type == 1)
            {
                var exp = ExpHelper.Create<CON_AreaCode>(m => m.AreaLevel == level);

                if (level > 1) exp = exp.And(m => m.ParentAreaCode == areaCode);

                retList.AddRange(ConAreaCode.GetModels(exp));
            }
